                ...
                <div class="card-body">
                    <form action="{{ route('events.update', $event_data['id']) }}" method="POST">
                        ...
                        <div class="row mb-3">
                                ...
                                <label class="col-sm-2 col-form-label" for="basic-default-date2">Logistics</label>
                                <div class="col-sm-10 my-auto" onclick="ADDLOGISTICSFORM()" style="cursor: pointer;">
                                    <i class="fa fa-plus-circle " aria-hidden="true" style="font-size: 20px;color:black;" ></i><span style="font-size: 20px;color:black;">Add More</span>
                                </div>
                            
                        </div>
                        @if ($event_data['Haslogistics'])
                            
                            @foreach ($event_data['Eventlogistics'] as $key=> $data)
                                <div class="row mb-3" id="existing_logistics_data-{{ $key }}">
                                    <div class="col-sm-2">
                                        <p> </p>
                                    </div>
                                    <div class="col-sm-3">
                                        <input type="text" class="form-control" name="item_name[]" value="{{ $data->item ?? 0 }}"  placeholder="Logistics Name" />
                                    </div>
                                    <div class="col-sm-3">
                                        <input type="number" class="form-control" id="basic-default-time2" name="item_quantity[]" value="{{ $data->quantity ?? 0 }}" placeholder="Logistics quantity" />
                                    </div>
                                    <div class="col-sm-3 my-auto">
                                        <i class="fa fa-trash " aria-hidden="true" style="font-size: 25px;color:red;cursor:pointer;" onclick="REMOVEFORMEXIST('existing_logistics_data-{{ $key }}')"></i>
                                    </div>
                                </div>
                            @endforeach
